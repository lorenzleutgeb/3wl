...
\newcommand{\thesistitle}{Efficient Propagation for Lazy-Grounding Answer Set Programming} % The title of the thesis. The English version should be used, if it exists.
...
\settitle{\thesistitle}{Efficient Propagation for Lazy-Grounding Answer Set Programming} % Sets English and German version of the title (both can be English or German).
...
Without going into further details about how ASP works, consider the following example for an intuition.
...
%allowdisplaybreaks?
\begin{flalign}
&garment(winterjacket).\label{ex:garments:garments:begin}\\
&garment(jeans).\\
&garment(tshirt).\\
&garment(shorts).\label{ex:garments:garments:end}\\
&warm(winterjacket).\label{ex:garments:style:begin}\\
&light(shorts).\label{ex:garments:style:end}\\
summer.\label{ex:garments:season}\\
...
In lines \ref{ex:garments:garments:begin} to \ref{ex:garments:garments:end} we list the five pieces we have in the wardrobe (a winter jacket, a pair of jeans, a t-shirt and shorts) and in lines \ref{ex:garments:style:begin} to \ref{ex:garments:style:end} garments that are warm or light are qualified. The current season is encoded in line \ref{ex:garments:season}. Using the two inference rules in lines \ref{ex:garments:inseason:begin} and \ref{ex:garments:inseason:end} we define when a garment is \enquote{in season}, i.e.~whether a piece is not light (resp.~not warm) which means it should be worn in winter (resp.~summer). Finally, we express that a garment can be worn in case it is in season on line \ref{ex:garments:wear}. Line \ref{ex:garments:seasons} models a constraint: It is unreasonable that it is both winter and summer for one evaluation of the program.
\end{example}

The evaluation of an ASP program is done by an ASP system, which may be comprised of further components. For \enquote{user} and programmer, no detailed knowledge about the system is required. The abstract input/output behaviour of these systems is enough to use them in most cases: Given an input program (and sometimes a few options on which algorithms or data structures to employ) it computes the answer sets.

% benuetzen gleiche techniken, es hat sich gezeigt, dass techniken aus dem sat bereich effizient sind

Many ASP system implementations use solving algorithms closely related to those found in SAT solvers to search for answer sets. Adapting SAT solvers turned out to be highly effective for solving ASP. Because there are no variables in SAT, variables such as \emph{G} in the above example must be removed from the program. The process of substituting variables with constants, e.g. substituting \emph{G} with one of the four concrete garments, is referred to as \emph{grounding} and the resulting variable-free program is said to be \emph{ground}.
\begin{example}
\label{ex:garmentsground}
Reconsider Example \ref{ex:garments}. Naively substituting each occurrence of \emph{G} in the example yields eight instances of the \emph{inseason} rule (two per garment), and four of the \emph{wear} rule (one per garment).
\begin{flalign}
&garment(winterjacket).\label{ex:garmentsground:garments:begin}\\
&garment(jeans).\\
&garment(tshirt).\\
&garment(shorts).\label{ex:garmentsground:garments:end}\\
&warm(winterjacket).\label{ex:garmentsground:style:begin}\\
&light(shorts).\label{ex:garmentsground:style:end}\\
summer.\label{ex:garmentsground:season}\\
inseason(winterjacket) \from& \ winter, garment(winterjacket), \fail light(winterjacket). \label{ex:garmentsground:inseason:begin}\\
...
inseason(winterjacket) \from& \ summer, garment(winterjacket), \fail warm(winterjacket).\\
...
After an analysis of 2WL, algorithms and data structures that account for lazy-grounding is devised and their soundness and completeness is shown. An implementation is contributed to the Alpha system, which in turn is compared against a naive approach in form of a benchmark.
...
Note that for atoms of arity zero, parentheses usually are omitted. For simplicity this definition of atoms does not account for function symbols or function terms. Grounding the program as briefly mentioned in Section \ref{chap:intro}, we formally define the class of ground atoms, for which terms must be constants.
...
Programs consist of rules, which in turn can be divided into head and body. The body of a rule can again be divided into a positive and a negative body.
...
\begin{definition}
	...
	A \emph{fact} is a rule $r$ with $\bodyf(r) = \emptyset$.
	
	A rule is \emph{ground} if all atoms in $\headf(r) \cup \bodyf(r)$ are.
\end{definition}
...
\begin{definition}
An \emph{interpretation} is a set of ground atoms $I$. All atoms contained in $I$ are said to be \emph{true}.
\end{definition}

%Consistency only for total interpretations, not needed anymore.
...
An interpretation $I$ is a \emph{(classical) model} of a ground rule $r$ if $\bodyf^+(r) \subseteq I$ and $\bodyf^-(r) \cap I = \emptyset$ imply that $\headf(r) \in I$, i.e.~whenever the body of $r$ is satisfied, then the head of $r$ is satisfied as well.
\end{definition}

The definition of a model generalizes to programs.

\begin{definition}
A \emph{(classical) model} of a ground program is an interpretation that is a model of all rules in the program.
\end{definition}
...
In this section we survey state-of-the-art ASP systems in order to highlight similarities and differences in their approaches. We focus on the comparison of ground-and-solve and lazy-grounding systems. \todo{Because this work is concerned with optimizing lazy-grounding systems using methods shown to be efficient in ground-and-solve systems.}

The first ASP systems developed are based on the ground-and-solve approach, as it is simpler and can be implemented in a more straight-forward manner than lazy-grounding. We chose to exhibit \clasp as it uses two-watched literals and employs nogood learning and \dlv because it introduced \mbt as a truth value, which turns out to be very effective in lazy-grounding as well.

Only gradually, running into the grounding bottleneck with benchmarks and in industry settings, the grounding bottleneck was recognized, which lead to the development of lazy-grounding systems. With lazy-grounding systems being newer, and less time spent reasoning about their efficiency and (re-)engineering them, they often fail to achieve the performance of ground-and-solve systems. The main culprit of early lazy-grounding systems is that they cannot leverage state-of-the-art techniques from ground-and-solve systems: Propagation using two-watched literals and heuristics are where the performance of ground-and-solve systems stems from.
...
Any newly enveloped ASP system should naturally profit from the design efforts and experimental results of the above systems (and others). We therefore aim to identify key features and techniques of these approaches.

\section{Two-Watched Literals}

As briefly mentioned in Section \ref{sec:motivation}, two-watched literals (2WL, \cite{effsat,questsat}) is an algorithm for unit propagation mainly used in SAT solvers. In this section we formally define nogoods and explain 2WL.

When grounding a program, rules can be rearranged to clauses that must not be satisfied as shown in Example \ref{ex:garmentprop}. Such clauses written as sets are called nogoods.

\begin{definition}
\label{def:nogood}
A \emph{nogood} is a set $\{ \bsgl_1, \ldots, \bsgl_n \}$ of literals.
\end{definition}

We will come back to this definition and extend it in section \ref{sec:nogoods}.

Nogoods are partial interpretations that cannot be extended to a solution. Given a nogood $\delta$ and an interpretation $I$, with $I \models \bsgl$ for all $\bsgl \in \delta$, then $I$ is not a solution. This property allows for propagation: If $T = \{ \bsgl \mid \bsgl \in \delta \text{ and } I \not \models \bsgl\}$ and $|T| = 1$ we say that $\delta$ is \emph{unit} or unit on $\bsgl$ for $\bsgl \in T$. In order to obtain a solution we may extend $I$ to $I'$ so that $I' \models \neg \bsgl$.

The number of nogoods generated from a program is proportional to the size of the corresponding ground program. Thus, one can expect many nogoods to be generated in the process. However, after constructing nogoods from (parts of) the input program and propagating truth values, it is important to efficiently re-evaluate whether there is a nogood that is unit under the extended assignment. This is where the 2WL strategy comes in. It descirbes an algorithm and associated data structure to efficiently track state of the nogoods.
...
\begin{example}
Consider Figure \ref{fig:2wl}. It shows the nogood $\delta = \{\bF a_1, \bT a_2, \bF a_3, \bT a_4 \}$ in the context of propagation using 2WL. The literals watching $\delta$ are marked by arrows. Literals that 
\begin{figure}
  \centering
...
\node(ng2des)[right=1cm of ng2.east, text width=7cm] { Pointers were not checked, as $a_3$ is not watching. };
...
\draw [<-] (ng4.three north) to node[auto] {} ++ (-0.1,0.5);
...
\chapter{Propagation for Lazy-Grounding Answer Set Programming}
\label{chap:prop}
...
In this chapter, we present algorithms and data structures involved in the propagation step of solving ASP programs. We adapt 2WL, a common strategy for propagation in SAT solves, and account for the truth value \mbt as motivated in Section \ref{sec:asperix}.
...
In this section we extends Definition \ref{def:nogood} and give associated definitions.
