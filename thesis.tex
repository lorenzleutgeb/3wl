...
%  }
  \ForEach{$\delta \in \Delta(\sigma)$}
  {
    ...
    \Else%If{there is some unassigned $\sigma' \in \delta$}
    {
      ...
      $\Delta(\wkn(\sigma')) \leftarrow \Delta(\wkn(\sigma')) \cup \{ \delta \}$\\
      $\Delta(\wkn(\sigma'')) \leftarrow \Delta(\wkn(\sigma'')) \cup \{ \delta \}$\\
    }
  }
  \Return{$(\mathbf{A}, \Delta)$}
...
In line 1, the input literal $\sigma$ is transformed to its weak form as lookups in $\Delta$ are always made against the set of nogoods that might propagate in case the literal \mbt, even if $\sigma$ is of $\bT x$. % Why?
...
In case (3.), $\Delta$ is modified in lines 8-11. As $\sigma$ is now assigned, it should not be watched for changes anymore. This is achieved by removing $\delta$ from the set of watches $\Delta(\sigma)$ and instead adding it to the set of watches for two unassigned literals in $\delta$.
