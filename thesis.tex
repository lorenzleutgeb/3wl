...
\texttt{clingo}\footnote{\url{https://potassco.org/clingo/}} is a ground-and-solve ASP system which combines the grounder \texttt{gringo} and the solver \texttt{clasp}. \clasp uses the 2WL strategy for propagation \cite[Sec.~5.3]{clasp} and implements conflict-driven nogood learning (CDNL) \cite[Sec.~4.1]{clasp} in analogy to conflict-driven clause learning (CDCL) in SAT solvers \cite{grasp,cdl-sat}.
...
\omiga \cite{omiga} uses computation to explore the search space similar to \asperix. In contrast, however, a Rete \cite{rete} network is used for propagation on rule heads. \omiga was extended for learning of non-ground rules in \cite{nglearn}, analogous to conflict driven learning as in \clasp.
