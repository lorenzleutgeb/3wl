...
In this section we detail Three Watched Literals. The process is split into four algorithms, varying in level of abstraction:
...
\paragraph{Grounding Explosion} This benchmark is modeled after the program shown in Example \ref{ex:explosion} and aims to exhibit the grounding bottleneck. Given a domain of size $n$, the problem is selecting at most one element from a domain of size $n$ and deriving an atom $p(X_1, \ldots , X_6)$ where all $X_1, \ldots , X_6$ are the selected element.
...
\paragraph{Cutedge} Taken from \cite[Example 1]{omiga}, this problem consists of computing reachability from a graph with exactly one edge removed.
